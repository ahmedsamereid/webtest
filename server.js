...
<p class="hint">عند النشر على الإنترنت (IP عام)، تظهر بيانات IP Geolocation تلقائيًا. على localhost استخدم Geolocation.</p>
</div>
</div>
<script>
...
const latEl=document.getElementById('lat');const lonEl=document.getElementById('lon');const lat=parseFloat(latEl.textContent);const lon=parseFloat(lonEl.textContent);if(!isNaN(lat)&&!isNaN(lon)){initMap(lat,lon);}
</script>
</body>
</html>`);
});
